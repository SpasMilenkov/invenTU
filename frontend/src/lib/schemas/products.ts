...
const baseObject = z.object({
  ...
  unitPrice: z.number({ message: 'Unit price is required' }).gt(0, 'Must be greater than 0'),
  costPrice: z.number({ message: 'Cost price is required' }).gte(0, 'Must be 0 or greater'),
  unitOfMeasure: z.string().min(1, 'Unit of measure is required').max(20, 'Max 20 characters'),
  isActive: z.boolean(),
  description: z.string().max(2000, 'Max 2000 characters').optional(),
  barcode: z.string().max(100, 'Max 100 characters').optional(),
  minStockLevel: z.number().int('Must be a whole number').gte(0, 'Must be 0 or greater'),
  maxStockLevel: z.number().int('Must be a whole number').gte(0, 'Must be 0 or greater').nullish(),
  reorderPoint: z.number().int('Must be a whole number').gte(0, 'Must be 0 or greater'),
});
...
export function normalizeUpdatePayload(values: UpdateProductInput): UpdateProductInput {
  return {
    ...values,
    primaryWarehouseId:
